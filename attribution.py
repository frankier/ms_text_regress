...
model = BertForMultiScaleSequenceRegression.from_pretrained(checkpoint, device_map=device)
print("Model loaded")
model.eval()
model.zero_grad()
...
def explain(text, model, tokenizer, int_bs, n_steps=50):
    ...
    return (
        get_word_at(tokens, worst_token),
        get_word_at(tokens, best_token)
    )
...
for row in dataset[split]:
    ...
    print(f"{row_idx}/{num_rows} worst {worst} {SWN_KEY[worst_swn]}\tbest {best} {SWN_KEY[best_swn]}")
