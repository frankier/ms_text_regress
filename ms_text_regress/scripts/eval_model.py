...
from ms_text_regress.scripts.train import (
    ExtraArguments,
    HfArgumentParser,
    TrainerAndEvaluator,
    TrainingArguments,
...
def main():
    training_args, extra_args, load_args = TrainerAndEvaluator.get_args(
        parser=HfArgumentParser((TrainingArguments, ExtraArguments, LoadArguments))
    )
    ...
    with ensure_context(pool) as pool_init:
        ...
        metrics = trainer.evaluate(
            traineval.eval_dataset["validation"], metric_key_prefix="eval"
        )
        ...
        metrics = trainer.evaluate(
            traineval.eval_dataset["test"], metric_key_prefix="test"
        )
        print("Test metrics")
        if "label_dists" in metrics:
            del metrics["label_dists"]
